...
public class Button extends Sprite {
	...
	public static function Create(title:String, style:Object, styleText:Object):Button {
		return new Button(title, style.width, style.height, style.deltaWidth, style.deltaHeight, style.colorBg,
				style.colorBgOver, style.colorBgDown, style.alphaBg, style.thicknessBorder, style.colorBorder,
				style.sizeRound, style.blurX, style.blurY, style.styleGlowUp, style.styleGlowOver, style.styleGlowDown,
				styleText);
	}

	public function Button(title:String, width:int = 0, height:int = 0, deltaWidth:int = 20, deltaHeight:int = 12,
						   colorBg:uint = 0x673B15, colorBgOver:uint = 0xFF6600, colorBgDown:uint = 0xFF6600,
						   alphaBg:Number = 0.9, thicknessBorder:int = 0, colorBorder:uint = 0x676D71, sizeRound:int = 14,
						   blurX:int = 0, blurY:int = 0, styleGlowUp:Object = null, styleGlowOver:Object = null,
						   styleGlowDown:Object = null, styleText:Object = null) {
		this.title = title;
...
class ButtonDisplayState extends Sprite {
	...
	public function ButtonDisplayState(colorBg:int, alphaBg:Number, border:Number, colorBorder:int, width:Number,
									   height:Number, sizeRound:int, blurX:int, blurY:int, styleGlow:Object = null) {
		...
		if (styleGlow) {
			gradientGlow = new GradientGlowFilter(styleGlow.distance, styleGlow.angle, styleGlow.colors, styleGlow.alphas,
					styleGlow.ratios, styleGlow.blurX, styleGlow.blurY, styleGlow.strength, BitmapFilterQuality.HIGH,
					styleGlow.type);
